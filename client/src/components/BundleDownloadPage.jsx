...
const BundleDownloadPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchBundle = async () => {
      try {
        const res = await fetch(`${apiBaseUrl}/files/bundle/${bundleCode}`, {
  ...
  const verifyBundle = async () => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/files/verifyBundlePassword`, {
        method: "POST",
  ...
  return (
    <section className="grid gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <article className="rounded-[32px] border border-mid bg-[linear-gradient(180deg,rgba(255,253,248,0.86),rgba(226,236,244,0.28))] p-5 sm:p-6">
        <div className="border-b border-mid pb-5">
          ...
          <h2 className="mt-2 text-2xl font-black tracking-tight text-ink sm:text-3xl">Shared file bundle</h2>
          <p className="mt-3 text-sm text-ink/75">
            {bundle.isPasswordProtected && !isVerified
              ? "Verify the shared password to reveal the files in this bundle."
              : "Every file from this upload is listed below and ready to open or download."}
          </p>
        </div>
