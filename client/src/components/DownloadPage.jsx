...
import { FaCalendarAlt, FaDownload, FaFileAlt, FaLock, FaUser } from "react-icons/fa";
...
const DownloadPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchFile = async () => {
      try {
        const res = await fetch(`${apiBaseUrl}/files/f/${shortCode}`, {
  ...
  const verifyFile = async () => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/files/verifyFilePassword`, {
        method: "POST",
  ...
  return (
    <section className="grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_360px]">
      <article className="overflow-hidden rounded-[32px] border border-mid bg-[linear-gradient(180deg,rgba(255,253,248,0.86),rgba(226,236,244,0.28))]">
        <div className="border-b border-mid px-6 py-5 sm:px-8">
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-ink/65">Shared file</p>
          ...
          <p className="mt-3 text-sm text-ink/75">
            {isProtected && !isVerified
              ...
              : "Preview the shared file before downloading it."}
          </p>
        </div>
          ...
          {isProtected && !isVerified ? (
            <div className="flex min-h-[420px] flex-col items-center justify-center rounded-[28px] border border-dashed border-mid bg-white/70 px-6 py-10 text-center">
              ...
              <p className="mt-2 max-w-md text-sm leading-6 text-ink/70">
                This file is protected. Enter the password below to unlock the preview and continue to download.
              </p>
            </div>
          ) : (
        ...
        <div>
          ...
          <div className="mt-4 grid gap-3">
            {detailItems.map(({ label, value, icon: Icon }) => (
              <div key={label} className="rounded-[22px] border border-mid bg-[linear-gradient(180deg,rgba(255,253,248,0.9),rgba(214,226,236,0.24))] p-4">
